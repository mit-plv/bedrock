...
Module M.
  ...
  Definition globalInv := Ex p, globalList =*> p * Ex ls, sll ls p.
End M.

Module T := Make(M).
Import M T.

Ltac sep := T.sep ltac:(unfold globalInv in *) SinglyLinkedList.hints.
  ...
  bmodule "test" {{
    bfunctionNoRet "handler"("sc", "i", "p", "r") [handlerS]
      ...
      While ("i" < 10) {
        ...
        [PREonly[V, R] [| R <> 0 |] * [| freeable R 2 |] * R =?> 2 * tq (V "sc") * globalInv * mallocHeap 0];;
      ...
      [PREonly[_, R] tq R * globalInv * mallocHeap 0];;
