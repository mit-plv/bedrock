...
(** CHANGED PIECE: Signature without hints *)
...
(** CHANGED PIECE: Hint-free version of standard library functor *)
Module OrderedTypeFacts (Import O: OrderedType).
Section hide_hints.
  Hint Immediate O.eq_sym.
  Hint Resolve O.eq_refl O.eq_trans O.lt_not_eq O.lt_trans.
...
Module KeyOrderedType(O:OrderedType).
 ...
 Notation key:=t.
  ...
  Instance lt_strorder : StrictOrder lt := MO.lt_strorder.
  Proof.
   split; eauto.
  ...
  Instance eq_equiv : Equivalence eq := MO.eq_equiv.
  Instance lt_compat : Proper (eq==>eq==>iff) lt := MO.lt_compat.
  Proof.
