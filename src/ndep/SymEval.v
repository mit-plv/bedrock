    ...
    Section WordAccess.
      ...
      Record SymEval_word (Predicate : ssignature wtypes (tvType pcIndex) (tvType stateIndex))
        ...
        ; sym_write_word : 
         ...
         ; sym_read_word_correct : forall funcs args uvars vars cs hyps pe ve m stn P,
         ...
         ; sym_write_word_correct : forall funcs args uvars vars P cs hyps pe ve v m stn args',
      ...
      Proof.
        ...
        eapply sym_read_word_correct 
          with (funcs := funcs) (uvars := uvars) (vars := vars0) (cs := cs) (stn := stn) (m := x4)
            in H6.
        3: instantiate (1 := sexprD funcs sfuncs uvars vars0 (Star
          (sheapD
            {|
              impures := FM.remove x (impures s);
              pures := pures s;
              other := other s |})
          (starred (Func x) x5 Emp))).
      ...
      Theorem symeval_write_word_correct : forall cs stn hyps funcs pe ve s s' uvars vars m,
      ...
      Proof.
        ...
        eapply fold_args_update_correct in H5.
        repeat match goal with
                 | [ H : exists x, _ |- _ ] => destruct H
               end. intuition; subst.
        generalize (sheapD_pures _ _ _ _ _ H3).
        rewrite sheapD_pull_impure in H3 by eauto.
        assert (In x7 (x5 ++ x7 :: x6)). 
        apply in_or_app; right; simpl; auto.
        eapply starred_In in H5.
        destruct H5. rewrite H5 in H3.
        rewrite <- heq_star_assoc in H3. rewrite heq_star_comm in H3.
        
        simpl in *. rewrite H1 in *.
        intros.
        eapply sym_write_word_correct in H2; eauto.
        ...
        generalize (sym_write_word_correct).
...
(*
    ...
    (** The full tactic **)
    (** TODO : I really want to abstract this so that I don't have to rewrite it for
     ** each read and write size
     **)
